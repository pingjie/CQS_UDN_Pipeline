...
processline <- function(x) {
  ...
  udnGT <- sapply(x[udnid], function(x) { switch(as.character(x), wdt = "○○", het = "●○", hom = "●●") })
  #homo to hom: UPDATED by Hui 05/28/2017
  ...
  if (x$Func.refGene == "exonic") {
    ...
    refseq <- detail[2]
  } else if (x$Func.refGene == "splicing" |
             x$Func.refGene == "UTR5" |
             x$Func.refGene == "UTR3" |
             x$Func.refGene == "ncRNA_splicing") {
  ...
  row1 <-
    c(
      ...
      udnGT,
      x$qual,
      round(x$g1kFreq, digits = 4),
      round(x$mis_z, digits = 2),
      rep(NA, 3),
      rank,
      comment
    )
...
convertFormat <- function(filename, platform) {
  ...
  ##ihMode type
  cs1 <-
    CellStyle(wb) + Fill(backgroundColor = "#d9d9d9", foregroundColor = "#d9d9d9") +
    Font(wb,
         heightInPoints = 20,
         isBold = TRUE,
         name = "Calibri")
  ##header ####
  cs2 <-
    CellStyle(wb) + Fill(backgroundColor = "#dce6f1", foregroundColor = "#dce6f1") +
    Alignment(h = "ALIGN_CENTER", vertical = "VERTICAL_CENTER") + Font(wb,
                                                                       heightInPoints = 12,
                                                                       isBold = TRUE,
                                                                       name = "Calibri") + Border(color = "#d9d9d9", position = c("LEFT", "RIGHT"))
  ##variants####
  cs3 <-
    CellStyle(wb) + Alignment(h = "ALIGN_CENTER", wrapText = T) + Font(wb,
                                                                       heightInPoints = 12,
                                                                       isBold = FALSE,
                                                                       name = "Calibri") + DataFormat("#,##0.00") ## UPDATED by Hui 06/02/2017 for wrapText
  startrow <- 1

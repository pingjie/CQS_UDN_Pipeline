appris <- function(step4file,apprisList='/gpfs21/scratch/cqs/udn/apprisP.refSeq.txt') {
	...
	step4$AAChange.refGene <- sapply(AAChange,getAppris,apprisList,'AAChange')	
        cat(sum(step4$Func.refGene=='exonic' & nchar(step4$GeneDetail.refGene)==0 & nchar(step4$AAChange.refGene)==0),'out of',nrow(step4),'rows were deleted due to non-APPRIS principals.\n')
	step4 <- step4[step4$Func.refGene!='exonic' | nchar(step4$GeneDetail.refGene)!=0 | nchar(step4$AAChange.refGene)!=0,]
